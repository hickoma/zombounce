...
namespace LeopotamGroup.Pooling {
    ...
    public sealed class PoolContainer : MonoBehaviour {
        ...
        public static PoolContainer CreatePool<T> (GameObject prefab, Transform itemsRoot = null) where T : IPoolObject {
            return CreatePool (prefab, itemsRoot, typeof (T));
        }
        ...
        public static PoolContainer CreatePool (GameObject prefab, Transform itemsRoot = null, Type overridedType = null) {
            if (prefab == null) {
                return null;
            }
            if (overridedType != null && !typeof (IPoolObject).IsAssignableFrom (overridedType)) {
                Debug.LogWarningFormat ("Invalid IPoolObject-type \"{0}\" for prefab \"{1}\"", overridedType, prefab.name);
                return null;
            }
            var container =
                new GameObject (
